...
export function FiveOclockSomewhere() {
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-400 via-orange-500 to-red-500 text-white p-8 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12 space-y-4">
          <h1 
            className={`text-4xl md:text-6xl font-bold text-shadow transition-all duration-1000 ease-out ${
          ...
          <p 
            className={`text-2xl md:text-3xl font-semibold text-yellow-200 transition-all duration-1000 ease-out delay-500 ${
              isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
            }`}
          >
        ...
        <Card className={`bg-white/20 backdrop-blur-lg shadow-xl mb-8 overflow-hidden transition-all duration-1000 ease-out ${
        ...
        <div className={`grid md:grid-cols-3 gap-8 mb-8 transition-all duration-1000 ease-out delay-300 ${
          isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}>
                ...
                {timeZones.map((zone, index) => (
                  <div
                    ...
                    className={`absolute w-4 h-4 rounded-full ${
                      index === currentZoneIndex ? 'bg-yellow-300 animate-ping' : 'bg-red-500'
                    }`}
                    style={{
        ...
        <div className={`grid md:grid-cols-2 gap-8 transition-all duration-1000 ease-out delay-500 ${
          isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}>
          <Card className="bg-white/20 backdrop-blur-lg shadow-xl">
            <CardHeader>
              ...
              <ol className="list-decimal list-inside space-y-2">
                {currentDrink.recipe.map((step, index) => (
                  <li key={index} className="flex items-start">
                    <GlassWater className="mr-2 h-5 w-5 mt-1 flex-shrink-0" />
                    <span>{step}</span>
                  </li>
                ))}
              </ol>
        ...
        <div className={`mt-8 text-center transition-all duration-1000 ease-out delay-700 ${
          isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}>
